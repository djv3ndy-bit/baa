...
function CafeDashboard({ counts }: { counts: DashboardCounts }) {
  return <>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel="View café plans" onPress={() => router.push('/subscription')} style={({ pressed }) => [s.blueprintPlan, pressed && s.pressed]}><View style={s.blueprintCrown}><Text style={s.crownText}>1</Text></View><View style={s.blueprintPlanBody}><Text style={s.planLabel}>YOUR CAFÉ PLAN</Text><View style={s.planNameRow}><Text style={s.blueprintPlanTitle}>Free</Text><View style={s.activePill}><Text style={s.activeText}>ACTIVE</Text></View></View><Text style={s.renewText}>Your first job and first hire are included.</Text></View><Text style={s.planArrow}>›</Text></Pressable>
