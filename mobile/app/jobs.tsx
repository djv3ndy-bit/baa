...
export default function JobsScreen() {
  ...
  return <SafeAreaView style={s.safe}>
    ...
    <ScrollView contentContainerStyle={s.wrap}><Pressable disabled={loading || busy} style={s.secondary} onPress={() => void load()}><Text style={s.secondaryText}>Refresh jobs</Text></Pressable>
      {loading ? <ActivityIndicator size="large" color="#321708" /> : error ? <Text accessibilityRole="alert" style={s.meta}>{error}</Text> : jobs.length ? jobs.map(job => <View key={job.id} style={s.card}><View style={s.cardTop}><View style={{ flex: 1 }}><Text style={s.jobTitle}>{job.title}</Text><Text style={s.meta}>{job.location || 'Location not added'} · {formatJobPay(job)}</Text><Text style={s.meta}>{job.schedule || 'Schedule not added'}</Text><Text style={s.meta}>{counts[job.id] || 0} applications retained</Text></View><View style={s.status}><Text style={s.statusText}>{job.active ? 'Active' : 'Paused'}</Text></View></View><View style={s.actions}>
...
const s = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#fbf7f1' }, headerCopy: { flex: 1, minWidth: 0 }, backButton: { width: 44, height: 44, flexShrink: 0, alignItems: 'center', justifyContent: 'center' }, header: { minHeight: 72, paddingVertical: 10, paddingHorizontal: 18, flexDirection: 'row', alignItems: 'center', gap: 12, borderBottomWidth: 1, borderBottomColor: '#eadfd5', backgroundColor: '#fff' }, back: { fontSize: 36, color: '#321708' }, title: { fontSize: 21, fontWeight: '900', color: '#321708' }, sub: { fontSize: 11, color: '#786a61', marginTop: 2 }, add: { flexShrink: 0, width: 44, height: 44, borderRadius: 14, backgroundColor: '#321708', alignItems: 'center', justifyContent: 'center' }, addText: { fontSize: 25, color: '#fff' }, wrap: { padding: 18, paddingBottom: 32 }, card: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#eadfd5', borderRadius: 18, padding: 16, marginTop: 14 }, cardTop: { flexDirection: 'row', gap: 10 }, jobTitle: { fontSize: 18, fontWeight: '900', color: '#321708' }, meta: { fontSize: 13, lineHeight: 20, color: '#746a61', marginTop: 6 }, status: { backgroundColor: '#eee8e3', borderRadius: 99, paddingHorizontal: 10, paddingVertical: 6, alignSelf: 'flex-start' }, statusText: { fontSize: 11, fontWeight: '900', color: '#41713c' }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: 9, marginTop: 14 }, secondary: { minHeight: 44, justifyContent: 'center', borderWidth: 1, borderColor: '#d6c2b3', borderRadius: 11, paddingHorizontal: 15, paddingVertical: 10, alignSelf: 'flex-start' }, secondaryText: { fontWeight: '900', color: '#321708' }, empty: { alignItems: 'center', paddingVertical: 70, paddingHorizontal: 24 }, emptyTitle: { fontSize: 25, fontWeight: '900', color: '#321708', marginTop: 15 }, primary: { backgroundColor: '#321708', borderRadius: 13, paddingHorizontal: 20, paddingVertical: 13, marginTop: 20, minHeight: 44 }, primaryText: { color: '#fff', fontWeight: '900' } });
