...
function JobsScreen() {
  const memory = useSectionMemory<{ jobs: MarketJob[]; counts: Record<string, number>; ownerId: string }>('jobs');
  const loaded = useRef(!!memory.initial);
  const [loading, setLoading] = useState(true), [jobs, setJobs] = useState<MarketJob[]>(memory.initial?.jobs ?? []);
  const [counts, setCounts] = useState<Record<string, number>>(memory.initial?.counts ?? {}), [error, setError] = useState(''), [busy, setBusy] = useState(false);
  const request = useRef(0), action = useRef(false), ownerId = useRef(memory.initial?.ownerId ?? ''), focused = useRef(false);
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (version !== request.current || !memory.current()) return;
      const nextCounts = applications.reduce<Record<string, number>>((result, item) => { result[item.job_id] = (result[item.job_id] || 0) + 1; return result; }, {});
      memory.save(user.id, role, { jobs: next, counts: nextCounts, ownerId: user.id });
      if (!memory.current()) return;
      loaded.current = true; ownerId.current = user.id; setJobs(next); setCounts(nextCounts);
    } catch (caught) { if (version === request.current && memory.current()) { memory.forget(); setJobs([]); setError(caught instanceof Error ? caught.message : 'Jobs could not load. Please refresh.'); } }
...
export default withSectionMemory(JobsScreen);
