...
function Matches() {
  ...
  const memory = useSectionMemory<{ rows: MatchRow[]; role: AppRole }>(`matches:${jobId || ''}`);
  const loaded = useRef(!!memory.initial);
  const [loading, setLoading] = useState(true), [rows, setRows] = useState<MatchRow[]>(memory.initial?.rows ?? []), [role, setRole] = useState<AppRole>(memory.initial?.role ?? 'barista'), [error, setError] = useState('');
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      memory.save(id, accountRole, { rows: [...mutual, ...legacy], role: accountRole });
      if (!memory.current()) return;
      loaded.current = true; setRole(accountRole); setRows([...mutual, ...legacy]);
    } catch (caught) { if (version === request.current && memory.current()) { memory.forget(); setRows([]); setError(caught instanceof Error ? caught.message : 'Matches could not load. Please refresh.'); } }
...
function useMatchScope() { return useLocalSearchParams<{ jobId?: string }>().jobId || ''; }
export default withSectionMemory(Matches, useMatchScope);
