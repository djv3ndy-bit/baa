...
export default function Matches() {
  ...
  return <SafeAreaView style={s.safe}><View style={s.header}><Text style={s.title}>Matches</Text><Text style={s.sub}>Both sides agreed — start a conversation</Text></View><ScrollView contentContainerStyle={s.list}>
    ...
    {loading ? <ActivityIndicator size="large" color="#321708"/> : error ? <Text accessibilityRole="alert" style={s.emptyCopy}>{error}</Text> : rows.length ? rows.map(row => <Pressable accessibilityRole="button" accessibilityLabel={`Message ${row.name} about ${row.detail}`} key={`${row.kind}-${row.id}`} style={s.row} onPress={() => router.push({ pathname: '/chat/[id]', params: { id: row.id, kind: row.kind } })}><View style={{ flex: 1 }}><Text style={s.name}>{row.name}</Text><Text style={s.meta}>{row.detail} · Matched</Text></View><Text style={s.chev}>›</Text></Pressable>) : <View style={s.empty}><Text style={s.emptyTitle}>No matches yet</Text><Text style={s.emptyCopy}>Review incoming profile interests in Discover, or connect through a job application.</Text><Pressable style={s.refresh} onPress={() => router.push({ pathname: '/discover', params: { tab: 'received' } })}><Text style={s.name}>Review profile interests</Text></Pressable></View>}
  </ScrollView><AppBottomNav active="matches" role={role}/></SafeAreaView>;
}
const s = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#fbf7f1' }, header: { padding: 20, paddingBottom: 10 }, title: { fontSize: 31, fontWeight: '900', color: '#321708' }, sub: { fontSize: 14, color: '#746a61', marginTop: 4 }, list: { padding: 18, paddingBottom: 30 }, refresh: { minHeight: 44, padding: 12, borderWidth: 1, borderColor: '#d6c2b3', borderRadius: 12, marginBottom: 14, marginTop: 10 }, row: { flexDirection: 'row', alignItems: 'center', gap: 13, backgroundColor: '#fff', borderWidth: 1, borderColor: '#eadfd5', borderRadius: 18, padding: 15, marginBottom: 10 }, name: { fontSize: 16, fontWeight: '900', color: '#24150d' }, meta: { fontSize: 12, color: '#746a61', marginTop: 4 }, chev: { fontSize: 29, color: '#a95820' }, empty: { alignItems: 'center', paddingTop: 65 }, emptyTitle: { fontSize: 25, fontWeight: '900', color: '#321708', marginTop: 15 }, emptyCopy: { fontSize: 14, lineHeight: 20, textAlign: 'center', color: '#746a61', marginTop: 6 } });
